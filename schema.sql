...
--    file_id INT NOT NULL REFERENCES gtfs_test.real_gtfs_files(file_id),
--   	file_name TEXT,
--    column_name TEXT,
--    value TEXT
--);
...
-- THESE TABLES DON'T HAVE DEPENDENCIES TO OTHER TABLES SO THEY CAN BE CREATED FIRST
...
CREATE TABLE gtfs_test.real_transitland_stops (
	...
	zone_id TEXT,
	stop_url TEXT,
	location_type SMALLINT REFERENCES gtfs_test.e_location_types(location_type),
	parent_station TEXT,
	stop_timezone TEXT,
	wheelchair_boarding SMALLINT REFERENCES gtfs_test.e_wheelchair_boardings(wheelchair_boarding),
	level_id TEXT,
	platform_code TEXT,
	PRIMARY KEY(file_id, stop_id)
	-- FOREIGN KEY(file_id, parent_station) REFERENCES gtfs_test.real_transitland_stops(file_id, stop_id),
	-- FOREIGN KEY(file_id, level_id) REFERENCES gtfs_test.real_transitland_levels(file_id, level_id)
);
-- SELECT * FROM gtfs_test.transitland_stops
...
CREATE TABLE gtfs_test.real_transitland_routes (
	...
	-- FOREIGN KEY(file_id, agency_id) REFERENCES gtfs_test.real_transitland_agency(file_id, agency_id)
);
-- SELECT * FROM gtfs_test.transitland_routes
...
CREATE TABLE gtfs_test.real_transitland_calendar_dates (
	file_id INT NOT NULL REFERENCES gtfs_test.real_gtfs_files(file_id),
	service_id TEXT, -- REFERENCES real_transitland_calender(service_id) OR this table's service_id
	date DATE NOT NULL, 
...
CREATE TABLE gtfs_test.real_transitland_fare_rules (
	...
	-- FOREIGN KEY(file_id, fare_id) REFERENCES gtfs_test.real_transitland_fare_attributes(file_id, fare_id)
	-- FOREIGN KEY(file_id, route_id) REFERENCES gtfs_test.real_transitland_routes(file_id, route_id)
	-- FOREIGN KEY(file_id, origin_id) REFERENCES gtfs_test.real_transitland_stops(file_id, zone_id)
	-- FOREIGN KEY(file_id, destination_id) REFERENCES gtfs_test.real_transitland_stops(file_id, zone_id)
	-- FOREIGN KEY(file_id, contains_id) REFERENCES gtfs_test.real_transitland_stops(file_id, zone_id)
);
-- SELECT * FROM gtfs_test.transitland_fare_rules
...
CREATE TABLE gtfs_test.real_transitland_trips (
	...
	block_id TEXT,
	shape_id TEXT,
	wheelchair_accessible SMALLINT REFERENCES gtfs_test.e_wheelchair_accessibility(wheelchair_accessibility),
	bikes_allowed SMALLINT REFERENCES gtfs_test.e_bikes_allowed(bikes_allowed),
	PRIMARY KEY(file_id, trip_id)
	-- FOREIGN KEY(file_id, route_id) REFERENCES gtfs_test.real_transitland_routes(file_id, route_id)
	-- FOREIGN KEY(file_id, shape_id) REFERENCES gtfs_test.real_transitland_shapes(file_id, shape_id)
);
-- SELECT * FROM gtfs_test.transitland_trips
...
CREATE TABLE gtfs_test.real_transitland_stop_times (
	...
	drop_off_type SMALLINT REFERENCES gtfs_test.e_drop_off_types(drop_off_type),
	continuous_pickup SMALLINT REFERENCES gtfs_test.e_continuous_pickup(continuous_pickup),
	continuous_drop_off SMALLINT REFERENCES gtfs_test.e_continuous_drop_off(continuous_drop_off),
	shape_dist_traveled FLOAT,
	timepoint SMALLINT REFERENCES gtfs_test.e_timepoints(timepoint),
	PRIMARY KEY(file_id, trip_id, stop_sequence)
	-- FOREIGN KEY(file_id, trip_id) REFERENCES gtfs_test.real_transitland_trips(file_id, trip_id)
	-- FOREIGN KEY(file_id, stop_id) REFERENCES gtfs_test.real_transitland_stops(file_id, stop_id)
);
-- SELECT * FROM gtfs_test.transitland_stop_times
...
CREATE TABLE gtfs_test.real_transitland_fare_products (
	...
	-- FOREIGN KEY(file_id, fare_media_id) REFERENCES gtfs_test.real_transitland_fare_media(file_id, fare_media_id)
);
-- SELECT * FROM gtfs_test.transitland_fare_products
...
CREATE TABLE gtfs_test.real_transitland_fare_leg_rules (
	...
	-- FOREIGN KEY(file_id, from_area_id) REFERENCES gtfs_test.real_transitland_areas(file_id, area_id)
	-- FOREIGN KEY(file_id, to_area_id) REFERENCES gtfs_test.real_transitland_areas(file_id, area_id)
	-- FOREIGN KEY(file_id, from_timeframe_group_id) REFERENCES gtfs_test.real_transitland_timeframes(file_id, timeframe_id)
	-- FOREIGN KEY(file_id, to_timeframe_group_id) REFERENCES gtfs_test.real_transitland_timeframes(file_id, timeframe_id)
	-- FOREIGN KEY(file_id, fare_product_id) REFERENCES gtfs_test.real_transitland_fare_products(file_id, fare_product_id)
);
-- SELECT * FROM gtfs_test.transitland_fare_leg_rules
...
CREATE TABLE gtfs_test.real_transitland_fare_transfer_rules (
	...
	-- FOREIGN KEY(file_id, from_leg_group_id) REFERENCES gtfs_test.real_transitland_fare_leg_rules(file_id, leg_group_id)
	-- FOREIGN KEY(file_id, to_leg_group_id) REFERENCES gtfs_test.real_transitland_fare_leg_rules(file_id, leg_group_id)
	-- FOREIGN KEY(file_id, fare_product_id) REFERENCES gtfs_test.real_transitland_fare_products(file_id, fare_product_id)
);
-- SELECT * FROM gtfs_test.transitland_fare_transfer_rules
...
CREATE TABLE gtfs_test.real_transitland_stop_areas (
	...
	-- FOREIGN KEY(file_id, area_id) REFERENCES gtfs_test.real_transitland_area(file_id, area_id)
	-- FOREIGN KEY(file_id, stop_id) REFERENCES gtfs_test.real_transitland_stops(file_id, stop_id)
);
-- SELECT * FROM gtfs_test.transitland_stop_areas
...
CREATE TABLE gtfs_test.real_transitland_route_networks (
	...
	route_id TEXT NOT NULL,
	PRIMARY KEY(file_id, route_id)
	-- FOREIGN KEY(file_id, network_id) REFERENCES gtfs_test.real_transitland_networks(file_id, network_id)
	-- FOREIGN KEY(file_id, route_id) REFERENCES gtfs_test.real_transitland_routes(file_id, route_id)
);
-- SELECT * FROM gtfs_test.transitland_route_networks
...
CREATE TABLE gtfs_test.real_transitland_transfers (
	...
	-- FOREIGN KEY(file_id, from_stop_id) REFERENCES gtfs_test.real_transitland_stops(file_id, stop_id)
	-- FOREIGN KEY(file_id, to_stop_id) REFERENCES gtfs_test.real_transitland_stops(file_id, stop_id)
	-- FOREIGN KEY(file_id, from_route_id) REFERENCES gtfs_test.real_transitland_routes(file_id, route_id)
	-- FOREIGN KEY(file_id, to_route_id) REFERENCES gtfs_test.real_transitland_routes(file_id, route_id)
	-- FOREIGN KEY(file_id, from_trip_id) REFERENCES gtfs_test.real_transitland_trips(file_id, trip_id)
	-- FOREIGN KEY(file_id, to_trip_id) REFERENCES gtfs_test.real_transitland_trips(file_id, trip_id)
);
-- SELECT * FROM gtfs_test.transitland_transfers
...
CREATE TABLE gtfs_test.real_transitland_pathways (
	...
	pathway_mode SMALLINT NOT NULL REFERENCES gtfs_test.e_pathway_modes(pathway_mode),
